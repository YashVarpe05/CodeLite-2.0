...
export default function InfiniteHero() {
	...
	useGSAP(
		() => {
			...
			function startLoop() {
				// Kill any existing loop to avoid duplicates (hot reload safety)
				if (bgRef.current.__loopTween) {
					bgRef.current.__loopTween.kill();
				}
				bgRef.current.__loopTween = gsap.to(bgRef.current, {
					keyframes: [
						{ scale: 1.025, xPercent: 1 },
						{ scale: 1.045, xPercent: -1.5 },
						{ scale: 1.03, xPercent: 0 },
					],
					duration: 18,
					repeat: -1,
					ease: "sine.inOut",
					// Use yoyo for smoother reversal feel across keyframes loop
					yoyo: true,
					force3D: true,
				});
			}
			...
			return () => {
				h1Split.revert();
				pSplit.revert();
				if (bgRef.current && bgRef.current.__loopTween) {
					bgRef.current.__loopTween.kill();
				}
			};
		},
		...
		>
			...
			<div className="relative  flex h-svh w-full items-center justify-center px-6">
				<div className="text-center">
					<h1
						ref={h1Ref}
						className="mx-auto max-w-2xl lg:max-w-4xl text-[clamp(2rem,3vw,3rem)] font-extralight leading-[0.95] tracking-tight"
					>
					...
					>
						Unleash your boldest ideas in a 24-hour odyssey of creation, where
						visionaries gather—guided by the light of Athena, Astra, and
						Aegis—to forge the technologies of tomorrow.
					</p>
